...
const processor = async (job) => {
    ...
    const videoId = await DownloadFromB2({
        key: job.data.key, onProgress: async (percent, loaded, total) => {
            await job.updateProgress({ type: "downloading", fileName: job.data.key, percent, loaded, total });
        }
    });
    console.log(`[WORKER] File downloaded to: ${videoId}`);
    // Step 2 : Process the video File on server (worker)
    await processVideo(`./downloads/${videoId}`, async ({ res, progress }) => {
        await job.updateProgress({ type: "processing", res, progress });
    });
    // Optionally delete the local file after processing
    try {
        await fs.unlink(`./downloads/${videoId}`);
        console.log(`[WORKER] Deleted file: ./downloads/${videoId}`);
    } catch (err) {
        console.warn(`[WORKER] Could not delete file ./downloads/${videoId}: ${err.message}`);
    }
    ...
    const uploadResult = await UploadOnB2(videoId.split('.')[0], true, async (info) => {
        await job.updateProgress({ type: "uploading", ...info });
    });
